      ...
      .then(() => {
        ...
        setTimeout(() => {
          if (!finished) {
            ...
            const inFlightIds = Object.keys(tab.prerender.requests || {});
            if (inFlightIds.length) {
              const lines = inFlightIds.map((id) => {
                const url = tab.prerender.requests[id];
                const entry = tab.prerender.pageLoadInfo.entries[id];
                const isInitial = id === tab.prerender.initialRequestId;
                const gotResponse = !!(entry && entry.responseParams);
                return `  [reqId=${id}${isInitial ? ' INITIAL' : ''} gotResponse=${gotResponse}] ${url}`;
              });
              util.log(
                `requestsInFlight for url=${tab.prerender.url}:\n${lines.join('\n')}`,
              );
            }
